...
//static volatile char        TEST_PKT[6] = {-128, 255, -127, 0, 0, 255};
...
int main( void )
{
    ...
    TA0CCR0 = (uint16_t) (32768-1);    // Start timer count up

    __enable_interrupt();   // Enables interrupts and enter LPM0
...
//
//    UCB0TXBUF   = SIG_PTH_RST;      // Configure Signal Path Reset Reg.
//    while (!(UCB0IFG & UCTXIFG));
...
//    while (!(UCB0IFG & UCTXIFG));
//
//    UCB0TXBUF   = PWR_MGMT_1;       // Configure Power Mgmt. Reg. 1
//    while (!(UCB0IFG & UCTXIFG));
//    UCB0TXBUF   = CLKSEL_4 | CYCLE | TEMP_DIS;
//    while (!(UCB0IFG & UCTXIFG));
//    UCB0CTL1    |= UCTXSTT;
//    UCB0IFG     &= ~UCTXIFG;
//    while (!(UCB0IFG & UCTXIFG));
//
//    UCB0TXBUF   = PWR_MGMT_2;       // Configure Power Mgmt. Reg. 2
//    while (!(UCB0IFG & UCTXIFG));
//    UCB0TXBUF   = STBY_G | LP_3;
//    while (!(UCB0IFG & UCTXIFG));
//    UCB0CTL1    |= UCTXSTT;
//    UCB0IFG     &= ~UCTXIFG;
//
//    /* Set I2C to Controller Receive */
//    ACL_ADDR    = ACCEL_STT;
//
//    while (!(UCB0IFG & UCTXIFG));
//    UCB0TXBUF   = ACL_ADDR;        // Point to start of ACL_data reg.
//    while (!(UCB0IFG & UCTXIFG));
//    UCB0CTL1    &= ~UCTR;
...
//    while (UCB0CTL1 & UCTXSTT);
    ...
    I2C_write(PWR_MGMT_1, (CLKSEL_4 | CYCLE | TEMP_DIS));
    I2C_write(PWR_MGMT_2, (STBY_G | LP_3));
